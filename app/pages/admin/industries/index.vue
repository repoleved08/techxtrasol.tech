...
const { getIndustries, createIndustry, updateIndustry, deleteIndustry } = useTaxonomy()
...
onMounted(async () => {
  try { items.value = await getIndustries() }
...
async function handleSave() {
  if (editId.value) await updateIndustry(editId.value, { ...form })
  else await createIndustry({ ...form })
  items.value = await getIndustries()
...
<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-bs-foreground-light">Industries</h1>
        <p class="text-sm text-bs-foreground-dark mt-1">Industries your clients operate in</p>
      </div>
      <button @click="openNew" class="bs-btn inline-flex items-center gap-2">
        <Icon name="lucide:plus" class="w-4 h-4" /> New Industry
      </button>
    </div>
    ...
    <div v-if="showForm" class="bg-bs-surface-1 border border-bs-surface-3/50 rounded-xl p-6 mb-6">
      <h2 class="text-sm font-semibold text-bs-foreground-light mb-3">{{ editId ? 'Edit' : 'New' }} Industry</h2>
    ...
    <div v-if="!loading && items.length === 0 && !showForm" class="text-center py-20 bg-bs-surface-1 border border-bs-surface-3/50 rounded-xl">
      <Icon name="lucide:factory" class="w-16 h-16 text-bs-surface-3 mx-auto mb-4" />
      <p class="text-bs-foreground-dark mb-4">No industries yet</p>
      <button @click="openNew" class="bs-btn-sm">Add First Industry</button>
    </div>
